 ...
 */
 ...
 class ProximityEffect02 : MonoBehaviour
{
    ...
    private GameObject accelTrigger;
    ...
    private void Start()
    {
        ...
        accelTrigger = transform.GetChild(3).gameObject;
        accelTrigger.SetActive(false); // start inactive
    ...
    private void CheckColliderStatus()
    {
        if (!collisionsEnabled) // if collisions are disabled
        {
            if (currCoolDownValue < 1) // check if countdown timer is finished, re-enable
            {
                ...
                FXParticle01.Stop(); // discontinues particles
                FXParticle02.Stop(); // discontinues particles
                accelTrigger.SetActive(false); // disable accelTrigger
